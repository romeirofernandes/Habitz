...
export default function HabitTree({
  ...
  const heightClass = 
    streak >= 50 ? "h-96" :  // 24rem (384px) - Tailwind's largest standard size
    streak >= 30 ? "h-80" :  // 20rem (320px)
    streak >= 20 ? "h-72" :  // 18rem (288px)
    streak >= 10 ? "h-64" :  // 16rem (256px)
    "h-56";                  // 14rem (224px)
  
  // Width differences using standard Tailwind classes
  const widthClass = 
    streak >= 50 ? "w-96" :  // 24rem (384px) - Tailwind's largest standard size
    streak >= 30 ? "w-80" :  // 20rem (320px)
    streak >= 20 ? "w-72" :  // 18rem (288px) 
    streak >= 10 ? "w-64" :  // 16rem (256px)
    "w-56";
    ...
    <div className="absolute" style={{ 
      ...
      <div className={`relative ${widthClass} ${heightClass} ${progressClass}`}>
        <Spline 
          ...
          className="w-full h-full pointer-events-none" // Add pointer-events-auto here
        />
