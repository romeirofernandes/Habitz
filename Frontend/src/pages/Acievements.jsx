...
const Achievements = () => {
  ...
  const calculateStats = () => {
    // Extract categories
    const categories = [...new Set(habits.map(habit => habit.category || 'uncategorized'))];
    ...
    const perfectDays = Object.values(dateCount).filter(count => count === habits.length).length;
  ...
  const categories = ["all", "unlocked", "locked", "beginner", "streak", "completion", "variety", "time", "perfection"];
  
  const unlockedCount = achievements.filter(a => a.unlocked).length;
  const percentage = achievements.length ? Math.round((unlockedCount / achievements.length) * 100) : 0;
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        <div className="bg-[#0a0a0a] border border-[#222] rounded-xl p-6 mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
            <h2 className="text-xl font-bold mb-2 sm:mb-0">Your Achievement Progress</h2>
            <div className="bg-[#A2BFFE]/10 rounded-full px-4 py-1">
              <span className="text-sm font-medium text-[#A2BFFE]">{percentage}% Complete</span>
            </div>
          </div>
